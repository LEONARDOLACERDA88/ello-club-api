...
export class AdminService {
  ...
  async createFirstAdmin(email: string, password: string, name: string) {
    ...
    if (exists) throw new BadRequestException('Admin já existe')
  ...
  async getDashboard() {
    const [
      totalUsers, totalPartners, pendingPartners, activePartners,
      totalOffers, totalTransactions, revenueStats, topPartners, recentTransactions,
    ] = await Promise.all([
  ...
  async approvePartner(adminId: string, partnerId: string) {
    ...
    if (partner.status !== 'PENDING') throw new BadRequestException('Parceiro não está pendente')
  ...
  async suspendPartner(adminId: string, partnerId: string, reason?: string) {
    await this.prisma.partner.update({
      where: { id: partnerId },
  ...
  async payCommissions(adminId: string, partnerId: string) {
    const pending = await this.prisma.commission.findMany({
      where: { partnerId, status: 'PENDING' },
    })

    if (!pending.length) throw new BadRequestException('Nenhuma comissão pendente para este parceiro')
  ...
  async getReports() {
    ...
    const [monthlySavings, topOffers, categoryBreakdown, newUsersThisMonth, lgpdRequests] =
      await Promise.all([
        this.prisma.transaction.aggregate({
